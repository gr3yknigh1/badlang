...
void
darray_init(darray_t *out_darray, u64 item_capacity, usize item_size) {
    *out_darray = (darray_t){
...
void
darray_push(darray_t *darray, void *item) {
    ...
    noc_memory_copy(((char *)darray->items) + offset, item, darray->item_size);
    darray->item_count += 1;
}

void
darray_pop(darray_t *darray, void *out_item) {
    ...
    if (out_item != NULL) {
        noc_memory_copy(out_item,
                        ((char *)darray->items) +
                            darray->item_count * darray->item_size,
                        darray->item_size);
    }
}

void
darray_peek(const darray_t *darray, void const **const out_ptr_item) {
    if (darray->item_count == 0) {
        return;
    }
    size_t offset = DARRAY_FREE_SPACE_BEGIN(darray);
    *out_ptr_item = darray + offset - darray->item_size;
    //                                ^^^^^^^^^^^^^^^^^ - Substract size of last
    //                                item
}

void
darray_free(darray_t *darray) {
    free(darray->items);
    darray->items = NULL;
    darray->item_count = 0;
